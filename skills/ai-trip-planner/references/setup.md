...
| playwright（npm 包） | 浏览器自动化：点评/高德/携程/飞猪网页操作、小红书脚本 | 项目目录 `npm i playwright && npx playwright install chromium` | `node -e "import('playwright').then(()=>1)"` 在脚本运行目录执行不报错 | 五个平台通道全部降级（见降级矩阵），只剩搜索引擎与 WebFetch |
...
| 携程 | `https://www.ctrip.com` 右上角登录（扫码/短信，用户本人完成） | 打开 `hotels.ctrip.com` 右上角有账号名即活 | 公开列表与档期价多数可见；房型/取消政策/差评原文拿不全，产出标「未登录口径 · 待核实」 |
| 飞猪 | `https://www.fliggy.com` 右上角淘宝账号扫码 | 打开 `hotel.fliggy.com` 右上角有账号名即活 | 同上；淘宝侧改密会使登录态失效 |
...
| 携程/飞猪登录 | 公开列表与档期价（未登录口径） | 比价定住宿 | 标「未登录口径 · 待核实」；订房深链不需要登录，照常可出 |
| playwright | 五平台网页操作全停，WebFetch/搜索引擎兜底 | 全部平台通道 | 各通道按上述行降级 |
...
2. 五个平台登录：按上表「去哪登录」逐个扫码。小红书跑一次 `node scripts/search_ro.mjs "任意词"`，弹出的浏览器里登录，登录态会记住。
